...
public sealed class Lift13Tests
{
    private sealed record TestPerson(string Name, int Age, string ThirdValue, string FourthValue, string FifthValue, string SixthValue, string SeventhValue, string EighthValue, string NinthValue, string TenthValue, string EleventhValue, string TwelfthValue, string ThirteenthValue);
    ...
    public void Success()
    {
        ...
        var result =
            LiftResult<ValidationErrors>
                .Lift((string name, int age, string third, string fourth, string fifth, string sixth, string seventh, string eighth, string ninth, string tenth, string eleventh, string twelfth, string thirteenth) =>
                    new TestPerson(name, age, third, fourth, fifth, sixth, seventh, eighth, ninth, tenth, eleventh, twelfth, thirteenth))
                .Apply(nameResult)
                .Apply(ageResult)
                .Apply(thirdResult)
                .Apply(fourthResult)
        ...
        Assert.That(result.Value, Is.EqualTo(new TestPerson("Alice", 42, "Third value", "Fourth value", "Fifth value", "Sixth value", "Seventh value", "Eighth value", "Ninth value", "Tenth value", "Eleventh value", "Twelfth value", "Thirteenth value")));
    }

    [Test]
    public void SingleFailureOnThirteenth()
    {
        Result<string, ValidationErrors> nameResult = "Alice";
        Result<int, ValidationErrors> ageResult = 42;
        ...
        Result<string, ValidationErrors> eleventhResult = "Eleventh value";
        Result<string, ValidationErrors> twelfthResult = "Twelfth value";
        Result<string, ValidationErrors> thirteenthResult = new List<ValidationError> { new("Invalid value") };
